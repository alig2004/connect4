		...
		while (play == 3)
		{
			...
			{		
				...
				if (playery > 16) 
				{
					...
					//keeping track of where piece is with regards to column placement (<7 ensures piece stays within bounds)
					if (columnNum < 7)
					{
						columnNum++;
					}
			...
			{		
				...
				//ensuring piece doesn't go out of bounds
				if (playerx < 140)
				{
					hmoved = 1;
					hinverted=0;
					...
					//finding the row to place piece
					row = 0;
					...
					if (turn%2 == 1)
					{
						...
						//placing red piece on the board
						putImage(piecex, piecey, 16, 16, redPiece, 0, 0);

						//checking if red has connected 4
					...
					else
					{
						...
						//placing yellow piece on the board					
						putImage(piecex, piecey, 16, 16, yellowPiece, 0, 0);

						//checking if yellow has connected 4
...
int redcheck(int board[][COL], int row, int columnNum)
{
	...
	//horizontal check (beginning with left)
	...
	if ((columnNum >= 1) && (board[row][columnNum-1] == 1))
	{
		...
		if((columnNum >= 2) && (board[row][columnNum-2] == 1))
		{
			...
			else if((board[row][columnNum+1] == 1))
			{
				//clearing the screen
				fillRectangle(0,0,128, 160, 0x0); 
				return 1;
	...
	} //end horizontal leftt check


	//vertical check (starting below)
	...
	} //end right below check

	//diagonal check (left above)
...
int yellowcheck(int board[][COL], int row, int columnNum)
{
	...
	if ((columnNum >= 1) && (board[row][columnNum-1] == 2))
	{
		...
		if((columnNum >= 2) && (board[row][columnNum-2] == 2))
		{
			...
			else if((board[row][columnNum+1] == 2))
			{
				//clearing the screen
				fillRectangle(0,0,128, 160, 0x0); 
				return 2;
	...
	if ((row != 0) && (columnNum != 6) && (board[row-1][columnNum+1] == 2))
	{
		...
		if ((row < 3) && (columnNum < 5) && (board[row-2][columnNum+2] != 2))
		{
			...
			if ((row != 4) && (columnNum != 6) && (board[row+1][columnNum-1] == 2))
			{
				...
				//if not entered -> 2nd piece left above is not yellow, no win this turn
				else
				{
					return 3;
				}
	...
	else if((row != 4) && (columnNum != 0) && (board[row+1][columnNum-1]==2))
	{
		...
		if ((row < 3) && (columnNum > 1) && (board[row+2][columnNum-2])==2)
		{
			...
			//if not entered -> 4th piece left above is not red, no win this round
			else
			{
				return 3;
			}
...
int winPage(int red, int yellow, int play)
{
	...
	{
		printTextX2("Yellow is the", 0, 20, RGBToWord(0xff,0x00,0), 0);
